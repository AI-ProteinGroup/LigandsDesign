...
def eval_epoch(model, tokenizer, validation_data, device, arg):
...
def train(model, tokenizer, training_data, validation_data, optimizer, scheduler, device, arg):
    ...
    for epoch_i in range(arg['epoch']):
        ...
        ################# validating #################
        start = time.time()
        valid_loss, valid_accu = eval_epoch(model, tokenizer, validation_data, device, arg)
...
def main(arg:dict)->None:
    ...
    testset =  dataset.ProtransDataset(dataset_pkl['testset'] , protein_vocab, device)
    ...
    optimizer = optim.Adam(transformer.parameters(), lr = 1e-4, betas=(0.9, 0.98), eps=1e-09)
...
if __name__ == '__main__':
    arg = {}
    # base_config
    arg['data_pkl']               = 'data/tokenized/pair/dataset_token_token.pkl'
    arg['output_dir']             = 'save_folder/protrans_train/transformer_token_token_l0.0001_e30/'
    arg['use_tensorboard']        = False
    arg['save_mode']              = 'all'   # ['all', 'best']
